//! A standalone bench binary that runs the latency micro-benchmark and prints
//! percentiles to stdout so `cargo bench` displays them directly.
//! This bench is configured in Cargo.toml as a non-harness bench target.
...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    if !cfg!(feature = "rfengine") {
        println!("latency_runner: 'rfengine' feature not enabled; run with: cargo bench --features rfengine");
        return Ok(());
    }
    ...
    // Prepare HTML with multiple stylesheet links
    ...
    let cfg = rfheadless::EngineConfig {
        ...
        wait_for_stylesheets_on_load: false,
        ..Default::default()
    };
    ...
    println!(
        "[latency_percentiles] p50={}ms p95={}ms p99={}ms (threshold={}ms)",
