...
fn platform_noop_smoke() {
    ...
    d.set_metrics(DeviceMetrics { width: 360, height: 640, dpr: 3.0, touch: true });
    assert_eq!(d.metrics().width, 360);

    // service worker fetch
    let sw = p.service_worker_manager();
    let ev = FetchEvent { request_url: "https://example/".into(), method: "GET".into(), headers: Default::default() };
