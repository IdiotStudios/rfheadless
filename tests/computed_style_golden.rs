...
fn test_computed_style_golden() {
    let data = fs::read_to_string("tests/computed_style_golden.json").expect("Failed to read fixtures");
    ...
    for f in fixtures.as_array().unwrap() {
        ...
        let mut engine = RFEngine::new(rfheadless::EngineConfig::default()).expect("Failed to create RFEngine");
        engine.load_url(&url).expect("Failed to load URL");
        // Sanity check: ensure styles were extracted and passed to the harness
        let styles_check = engine.evaluate_script("(()=>{ return JSON.stringify(document.styles); })()").expect("Eval failed");
        if styles_check.value.trim() == "\"[]\"" || styles_check.value.trim().is_empty() {
            panic!("No styles found in harness for fixture; document.styles = {}", styles_check.value);
        }
        ...
        assert_eq!(val, expected.to_lowercase(), "Mismatch for selector {} property {} (document.styles={})", selector, property, styles_check.value);
    }
}
